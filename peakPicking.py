...
class SQLitePeakPickingManager:
    ...
    def load_dataframe(self, table_name):
        ...
        if 'peptide_id' in self.intensity_table.columns:
            self.intensity_table = self.intensity_table.rename(
                columns={'peptide_id': 'cluster'})
            self.intensity_table['cluster'] = self.intensity_table['cluster'].fillna(
                '0')

        self.intensity_table_pivot = pd.pivot_table(self.intensity_table.copy(), index=[
                                                    'protein_id', 'gene', 'cluster', 'sample', 'ptm', 'condition', 'replicate'], columns='fraction', values='intensity', fill_value=0).reset_index()
    ...
    def apply_smoothing(self, save_to_SQL=True):
        ...
        intensity_columns = self.intensity_table_pivot.copy(
        )[np.unique(self.intensity_table['fraction'])].columns
        ...
        smoothed_intensity_table[intensity_columns] = smoothed_intensity_table[intensity_columns].apply(
            smooth_intensities, axis=1)
        ...
        if save_to_SQL == True:
            ...
            smoothed_intensity_table.to_sql(
                smoothed_table_name, self.conn, index=False, if_exists='replace')
    ...
    def apply_peakpicking(self):
        ...
        def peakpicking(row):
            ...
            # get peaks
            pep_peaks = find_peaks(
                intensities, prominence=rel_threshold, distance=3, height=1000, width=1)[0]
            result = [[row['protein_id']]*len(pep_peaks), [row['gene']]*len(pep_peaks), [row['cluster']]*len(pep_peaks), [row['sample']]*len(pep_peaks), [
                row['ptm']]*len(pep_peaks), [row['condition']]*len(pep_peaks), [row['replicate']]*len(pep_peaks), pep_peaks, intensities[pep_peaks]]

            # get peak widths
            result.extend(
                list(peak_widths(intensities, pep_peaks, rel_height=0.3)))
        ...
        df_result = intensity_df.apply(
            peakpicking, axis=1, result_type='expand')

        # reshape peakpicking results into a peak_table
        df_result.columns = ['protein_id', 'genes', 'cluster', 'sample', 'ptm', 'condition', 'replicate', 'peak_apex',
                             "peak_height", "peak_width", "peak_width_height", "peak_left_ips", "peak_right_ips", 'peak_left', 'peak_right']
    ...
    def plot_multiplicity_distributions(self, file_location, file_string_add):
        ...
        for s in peak_table_summary['sample'].unique():
            ...
            peakMultiplicity_array = np.array(
                peak_table_summary_s['peak_apex'])
            mean_value = np.mean(peakMultiplicity_array)
            # peak_table_summary['peak_apex']
            counter = Counter(peakMultiplicity_array)
    ...
    def save_to_SQL(self, peak_table_name='peak_table'):
        ...
        self.peak_table.to_sql(peak_table_name, self.conn,
